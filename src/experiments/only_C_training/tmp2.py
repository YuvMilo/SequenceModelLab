...
def get_block_diag_vs_complex_training_tasks(
    ...
    def get_init_from_pretrained_name(name):
        path = os.path.join(".", "src", "experiments", "only_C_training",
                            "best_models", name + "_" + str(hidden_size))
        with open(path, "rb") as f:
            A = pickle.load(f)

        A = torch.tensor(A)

        def init(n, input_dim=1, output_dim=1):
            return A.clone(), B.clone(), C.clone(), D.clone()

        return init

    model_noise = get_flexible_ssm(init_func=get_init_from_pretrained_name("noise"),
                                   num_hidden_state=hidden_size,
                                   device=device,
                                   trainable_param_list=["C"])

    model_block_diag = get_flexible_ssm(init_func=get_init_from_pretrained_name("block_diag"),
                                        num_hidden_state=hidden_size,
                                        device=device,
                                        ssm_storing_strategy=BlockDiagStoringStrategy,
                                        trainable_param_list=["C"])

    model_block_diag_init = get_flexible_ssm(init_func=get_init_from_pretrained_name("block_diag_init"),
                                             num_hidden_state=hidden_size,
                                             device=device,
                                             trainable_param_list=["C"])

    model_complex_only_A = get_flexible_complex_diag_ssm(
        init_func=get_init_from_pretrained_name("complex_only_A"),
        num_hidden_state=hidden_size,
        device=device,
    ...
    logger_block_diag_init = SSMTrainingLogger(saving_path=block_diag_init_saving_path,
...
def get_ssm_rot_block_diag_vs_complex(
    ...
    rot_block_diag_model = get_flexible_ssm(init_func=get_init_from_pretrained_name("rot_block_diag"),
                                            num_hidden_state=hidden_size,
                                            device=device,
                                            ssm_storing_strategy=BlockDiagStoringStrategy,
                                            trainable_param_list=["C"])

    rot_block_diag_init_model = get_flexible_ssm(get_init_from_pretrained_name("rot_block_diag_init"),
                                                 num_hidden_state=hidden_size,
                                                 device=device,
                                                 trainable_param_list=["C"])

    model_complex_only_A = get_flexible_complex_diag_ssm(
        init_func=get_init_from_pretrained_name("rot_complex_only_A"),
        num_hidden_state=hidden_size,
        device=device,
        only_A_is_complex=True,
        trainable_param_list=["C"]
    ...
    return [task_block_diag_init, task_block_diag, task_complex_diag]
...
def run_exp():
    ...
    training_dict['BC_std'] = str(float(training_dict['BC_std']) ** 2)
    ...
    for i in range(times):
        for hidden in hiddens:
            savedir = os.path.join("..", "results",
                                   "only_C_trained_pretrainedA", str(hidden))
