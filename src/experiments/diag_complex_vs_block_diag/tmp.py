...
def get_block_diag_vs_complex_training_tasks(
    ...
    B = BC_std * torch.randn([hidden_size, 1], dtype=torch.float)
    ...
    logger_complex_ony_A = SSMTrainingLogger(saving_path=complex_only_A_saving_path,
    ...
    task_complex_only_A = train_smm_over_white_noise_lag_multiprocess.remote(
        model=model_complex_only_A,
        lag=lag,
        seq_len=seq_len,
        logger=logger_complex_ony_A,
        num_epochs=epochs,
        optimizer=opt,
        early_stop=False,
        min_cut=epochs,
    ...
    return [task_complex_only_A, task_block_diag]
...
def get_ssm_rot_block_diag_vs_complex(
    ...
    A, B, C, D = rot_block_diag_model.get_params()
    ...
    for i in range(times):
        for hidden in hiddens:
            savedir = os.path.join("..", "results",
                                   "block_diag_vs_complex", str(hidden))
